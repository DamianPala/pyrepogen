...
def upload_to_cloud(cwd='.', prompt=True):
    ...
    if prompt:
        if latest_release_package_path:
            if not wizard.is_checkpoint_ok(__name__, "Upload the {} package?".format(latest_release_package_path), ['y', 'n'], 'y'):
...
def download_package(cwd='.', package_name=None):
    ...
    if settings.RELEASE_PACKAGE_SUFFIX in package_name:
        ...
        raise exceptions.NameError("Incorrect package name!")
    ...
    with ftplib.FTP(credentials['server'], credentials['username'], credentials['password']) as ftp_conn:
        ...
        ftp_conn.cwd(bucket)
        if not package_name in ftp_conn.nlst():
            raise exceptions.FileNotFoundError("{} package not found on the cloud server".format(package_name))
    ...
    if (path_where_to_download).exists():
        _logger.info("File {} downloding to {} directory completeted.".format(package_name, path_where_to_download.parent))
    else:
        raise exceptions.FileNotFoundError("File {} downloading error! Please try again.".format(package_name))
...
def _get_cloud_project_bucket_path(credentials):
    return (Path('/') / credentials['main_bucket_path'] / credentials['client_name'] / credentials['project_name']).as_posix()
    ...
    if package_path:
        ...
        if package_name not in buckets_contents:
            ...
            if package_name in ftp_conn.nlst():
                _logger.info("File {} uploaded properly to directory {} of the cloud server!".format(package_name, ftp_conn.pwd()))
            else:
...
def _read_cloud_credentials(cwd='.'):
    ...
    if not file_path.exists():
        raise exceptions.FileNotFoundError("{} file not found!".format(settings.CLOUD_CREDENTIALS_FILENAME))
    ...
    for field in credentials_dict:
        if not credentials_dict[field]:
            raise exceptions.CredentialsError("{} field is empty in {} file!".format(field, settings.CLOUD_CREDENTIALS_FILENAME))
...
def _create_main_bucket_tree(ftp_conn, credentials):
    if not credentials['main_bucket_path'] in ftp_conn.nlst():
        raise exceptions.BucketNotFoudError("Bucket {} not found on server!".format(credentials['main_bucket_path']))
